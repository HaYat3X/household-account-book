export default function LoginPage() {
  return (
    <div className="min-h-screen bg-gradient-to-b from-green-50 to-white flex flex-col items-center justify-center px-6">
      <div className="w-full max-w-sm space-y-8">
        {/* Logo */}
        <div className="text-center space-y-3">
          <div className="mx-auto flex h-16 w-16 items-center justify-center rounded-2xl bg-green-600 shadow-lg shadow-green-200">
            <span className="text-3xl">🏠</span>
          </div>
          <div>
            <h1 className="text-2xl font-bold text-slate-900">家計簿</h1>
            <p className="mt-1 text-sm text-slate-500">2人で使う、シンプルな家計管理</p>
          </div>
        </div>

        {/* Features */}
        <ul className="space-y-3">
          {[
            { icon: "📷", text: "レシートを撮るだけで自動入力" },
            { icon: "📊", text: "カテゴリ別の予算を一目で確認" },
            { icon: "💸", text: "立替・精算をスムーズに管理" },
          ].map(({ icon, text }) => (
            <li key={text} className="flex items-center gap-3 text-sm text-slate-600">
              <span className="text-xl">{icon}</span>
              <span>{text}</span>
            </li>
          ))}
        </ul>

        {/* Login Button */}
        <div className="space-y-4">
          <button className="flex w-full items-center justify-center gap-3 rounded-xl bg-white px-5 py-3.5 text-sm font-medium text-slate-700 shadow-sm ring-1 ring-slate-200 active:bg-slate-50 transition-colors">
            <GoogleIcon />
            Googleでログイン
          </button>
          <p className="text-center text-xs text-slate-400">
            招待リンクをお持ちの方は、そちらからアクセスしてください
          </p>
        </div>
      </div>
    </div>
  );
}

function GoogleIcon() {
  return (
    <svg viewBox="0 0 24 24" className="h-5 w-5" aria-hidden="true">
      <path
        d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
        fill="#4285F4"
      />
      <path
        d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
        fill="#34A853"
      />
      <path
        d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
        fill="#FBBC05"
      />
      <path
        d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
        fill="#EA4335"
      />
    </svg>
  );
}
